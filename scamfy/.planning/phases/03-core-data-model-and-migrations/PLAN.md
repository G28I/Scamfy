...
- **Action**:
  ...
  - Create `backend/app/models/scam_pattern.py` & `backend/app/models/community_report.py`:
    - `ScamPattern`: Public/moderated indicator directory (`REP-01..03`). `id` (`UUID`), `indicator_type` (`String(32)`), `indicator_value` (`String(512)`, indexed), `category` (`String(64)`), `risk_level` (`String(32)`), `verification_status` (`String(32)`), `report_count` (`Integer`, default 1), `first_reported_at`, `last_reported_at`, `metadata_payload` (`JSONB`).
  ...
  - Create `backend/app/models/victim_case.py`:
    - `VictimCase`: `id` (`UUID`), `user_id` (FK to `users.id`, non-nullable owner), `title` (`String(255)`), `category` (`String(64)`), `financial_loss_amount` (`Numeric(12, 2)` nullable), `currency` (`String(3)`, default `'INR'`), `status` (`String(32)`: `DRAFT`, `OPEN`, `OFFICIAL_REPORTED`, `RESOLVED`, `ARCHIVED`), `official_complaint_ack_no` (`String(64)`), `support_grant_expires_at` (`DateTime(timezone=True)`).
  ...
  - Create `backend/app/models/audit_event.py` (`SEC-06`):
    - `AuditEvent`: Append-only audit record (no update/delete). `id` (`UUID`), `actor_id` (`String(128)`), `actor_role` (`String(32)`), `action` (`String(64)`), `target_resource_type` (`String(64)`), `target_resource_id` (`String(128)`), `details` (`JSONB` sanitized context metadata without raw secrets/PII), `ip_address_hash` (`String(64)`), `created_at` (`DateTime(timezone=True)`).
  - Export all 8 models in `backend/app/models/__init__.py`.
...
- **Action**:
  ...
  - Configure `backend/alembic/env.py` for async SQLAlchemy engine (`asyncpg`) and bind to `Base.metadata`.
  - Generate baseline migration `0001_initial_schema.py` creating all 8 tables, indexes, unique constraints, and foreign key cascades with bidirectional `upgrade()` and `downgrade()`.
...
- **Action**:
  - Configure `backend/tests/conftest.py` with async PostgreSQL test database engine (`TEST_DATABASE_URL`, e.g. `postgresql+asyncpg://postgres:postgres@localhost:5432/scamfy_test` or configured test container) that creates all tables in an isolated test schema/database.
  - Create `backend/tests/test_models.py`:
    ...
    - Test VictimCase, CaseTimelineEvent, and CaseEvidence ownership chain (`User -> VictimCase -> Evidence`) and cascade deletes.
    - Test AuditEvent append-only creation, indexed lookups, and ensure no update/delete semantics exist.
